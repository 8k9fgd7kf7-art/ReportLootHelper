javascript:(async()=>{
  ...
  if(screen!=='report'||(mode&&mode!=='view')){
  ...
  function getResourceInfo(){
    ...
    const row=rows.find(tr=>/ersp[aä]hte\s+rohstoffe/i.test(tr.textContent||''));
    if(!row) return null;
    const valueCell=row.querySelector('td:last-child')||row;
    let values=[...valueCell.querySelectorAll('[title],span')]
      .map(el=>parseGameNumber(el.textContent))
      .filter(value=>value>0);
    if(values.length<3){
      values=(valueCell.textContent.match(/\d[\d.\s]*/g)||[])
        .map(parseGameNumber)
        .filter(value=>value>0);
    }
