javascript:(async()=>{
  ...
  function render(){
    ...
    const reserveText=calc.settings.buffer>0
      ? `<br>Sicherheitsreserve ${calc.settings.buffer} % → benötigte Tragkraft: ${calc.wanted.toLocaleString('de-DE')}`
      : '';
    resultBox.innerHTML=`<b>${troopText||'Keine Truppen ausgewählt'}</b><br>Tragkraft: ${calc.totalCapacity.toLocaleString('de-DE')}<br>Rohstoffe: ${resources.total.toLocaleString('de-DE')}${reserveText}`;
  }
