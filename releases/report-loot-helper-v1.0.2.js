javascript:(async()=>{
  ...
  function getResourceInfo(){
    ...
    const labelCell=cells.find(cell=>labelPattern.test((cell.textContent||'').replace(/\s+/g,' ').trim()));
    ...
    let values=[...valueCell.querySelectorAll('.wood,.stone,.iron,[class*="wood"],[class*="stone"],[class*="iron"]')]
      .map(el=>parseGameNumber(el.textContent||el.getAttribute('title')))
      .filter(value=>value>0);
    if(values.length<3){
      values=(valueCell.textContent.match(/\d[\d.\s]*/g)||[]).map(parseGameNumber).filter(value=>value>0);
    }
    if(values.length<3){
      values=(row.textContent.match(/\d[\d.\s]*/g)||[]).map(parseGameNumber).filter(value=>value>0);
    }
